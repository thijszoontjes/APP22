...
export async function getUserLikedVideos(page: number = 1, pageSize: number = 20): Promise<LikedVideosResponse> {
  try {
    console.log('[Community] Fetching user liked videos, page:', page, 'pageSize:', pageSize);

    // Try /api/v1/likes first
    console.log('[Community] Trying endpoint: /api/v1/likes');
    let res = await communityFetch(`/api/v1/likes?page=${page}&page_size=${pageSize}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    console.log('[Community] /api/v1/likes returned status:', res.status);

    // If that doesn't work, try other endpoints
    if (!res.ok) {
      console.warn('[Community] /api/v1/likes returned', res.status, '- trying /likes');
      res = await communityFetch(`/likes?page=${page}&page_size=${pageSize}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        },
      });
      console.log('[Community] /likes returned status:', res.status);
    }

    if (!res.ok) {
      console.error('[Community] Could not fetch liked videos, status:', res.status);
      const text = await res.text();
      console.error('[Community] Response body:', text);
      return { items: [], total: 0, page, page_size: pageSize };
    }

    const data = await res.json();
    console.log('[Community] Raw response:', JSON.stringify(data, null, 2));
    
    // API returns likes with embedded video data
    // Extract videos from likes array
    const likes = data.likes || [];
    console.log('[Community] Found', likes.length, 'likes in response');
    
    const videosList = likes.map((like: any) => {
      console.log('[Community] Processing like for video:', like.video_id);
      return {
        id: like.video_id,
        video_id: like.video_id,
        title: like.video?.title || 'Video',
        description: like.video?.description,
        preview_image_url: like.video?.preview_image_url,
        aspect_ratio: like.video?.aspect_ratio,
        created_at: like.created_at,
        ...like.video,
      };
    });
    
    console.log('[Community] Extracted', videosList.length, 'videos from likes');
    ...
    console.error('[Community] Error fetching liked videos:', err?.message);
    console.error('[Community] Full error:', err);
    return { items: [], total: 0, page, page_size: pageSize };
  }
}
